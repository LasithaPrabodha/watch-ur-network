...
import { deepCloneObj, UserEntity } from '@wyn/ui-shared';
...
export class UsersApiService {
    ...
    return new Observable<UserEntity>((observer: Observer<UserEntity>) => {
      ...
      window.setTimeout(() => {
        ...
        newUser.friendNames.forEach((friendName: string) => {
          const friendUser = this.getUserByName(friendName);
          if (friendUser) {
      ...
      window.setTimeout(() => {
        ...
        const matchingUsers: UserEntity[] = this.users
          .filter((user: UserEntity) => {
            return (
              excludeFriendNames && !excludeFriendNames.includes(user.name)
            );
          })
          .filter((user: UserEntity) => {
  ...
  getFriendsGraph(): Observable<ForceDirectedGraph> {
    return new Observable<ForceDirectedGraph>(
      (observer: Observer<ForceDirectedGraph>) => {
        // This is where you'd normally have an httpClient.get() call, this timeout simulates it
        window.setTimeout(() => {
  ...
  private getUserByName(name: string): UserEntity | undefined {
